...
public class Preview extends ViewGroup implements SurfaceHolder.Callback {
    ...
    public void setCamera(Camera camera) throws IOException {
        ...
        if (mCamera != null) {
            ...
            try {
                ...
                List<String> mSupportedFocusModes = mCamera.getParameters().getSupportedFocusModes();
                if (mSupportedFocusModes.contains(Camera.Parameters.FOCUS_MODE_AUTO)) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        if (changed && getChildCount() > 0) {
            ...
            //Toast.makeText(getContext(), "TES Ali BABA!", Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
        if(mCamera != null) {
            ...
            int rotation = ((Activity) mContex).getWindowManager().getDefaultDisplay().getRotation();
            switch (rotation) {
